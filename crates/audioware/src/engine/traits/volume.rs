...
pub trait SetControlledVolume {
    fn set_controlled_volume(&mut self, id: ControlId, amplitude: Amplitude, tween: Tween);
}

impl<K, O, E> SetControlledVolume for DualHandles<K, O, E> {
    fn set_controlled_volume(&mut self, id: ControlId, amplitude: Amplitude, tween: Tween) {
...
impl<K, O> SetControlledVolume for Handles<K, StaticSoundHandle, O> {
    fn set_controlled_volume(&mut self, id: ControlId, amplitude: Amplitude, tween: Tween) {
        self.0
            .iter_mut()
...
impl<K, O, E> SetControlledVolume for Handles<K, StreamingSoundHandle<E>, O> {
    fn set_controlled_volume(&mut self, id: ControlId, amplitude: Amplitude, tween: Tween) {
        self.0
            .iter_mut()
            .filter(|x| x.control_id.map(|x| x == id).unwrap_or(false))
